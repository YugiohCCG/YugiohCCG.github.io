...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local announce_filter={TYPE_MONSTER,OPCODE_ISTYPE}
	local seen={}
	local g=Duel.GetFieldGroup(tp,LOCATION_MZONE,0)
	for tc in aux.Next(g) do
		local code=tc:GetCode()
		if code~=0 and not seen[code] then
			seen[code]=true
...
function s.efilter(e,ct)
	...
	if not te or not te:IsHasCategory(CATEGORY_SUMMON+CATEGORY_SPECIAL_SUMMON) then return false end
	local ac=e:GetLabel()
