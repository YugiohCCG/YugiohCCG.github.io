...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--If "Azrynior, the Abundaence of Purity" leaves the field because of your opponent's card
...
function s.fusfilter(c,e,tp,mg)
	return c:IsCode(CARD_AZRYNIOR) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false)
		and (not mg or mg:CheckSubGroup(s.matcheck,2,2,c,tp))
end
function s.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,TOKEN_PROTOGENIC,SET_TO_PROTO,TYPES_TOKEN_MONSTER,0,0,1,RACE_GALAXY,ATTRIBUTE_LIGHT)
...
function s.azfilter(c,tp)
	return c:IsCode(CARD_AZRYNIOR) and c:IsPreviousControler(tp)
		and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousPosition(POS_FACEUP)
		and (c:IsReason(REASON_BATTLE) or (c:IsReason(REASON_EFFECT) and c:GetReasonPlayer()==1-tp))
end
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.azfilter,1,nil,tp)
end
