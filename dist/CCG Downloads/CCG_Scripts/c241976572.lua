--Aquamarine Nautilus
local s,id=GetID()
local STRING_ID=133976572
local SET_AQUAMARINE=0xf3c
function s.initial_effect(c)
	--Fusion Material
	c:EnableReviveLimit()
	aux.AddFusionProcMix(c,true,true,aux.FilterBoolFunction(Card.IsSetCard,SET_AQUAMARINE),function(c) return c:IsAttribute(ATTRIBUTE_WATER) and c:IsRace(RACE_AQUA) end)
	--Send on Summon
