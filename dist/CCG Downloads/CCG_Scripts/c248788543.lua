--Right Talismandrake Arms - Blaze Sabre
local s,id=GetID()
local STRING_ID=132788543
...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Equip from GY during your Main Phase
	...
	c:RegisterEffect(e2)
	--Equip from GY if the opponent Special Summons
	local e3=e2:Clone()
...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_TALISMANDRAKE) and c:IsType(TYPE_MONSTER)
end
...
function s.negtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsOnField() and s.negfilter(chkc,e) end
...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsCanBeDisabledByEffect(e,false) then
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetCurrentPhase()&(PHASE_DAMAGE+PHASE_DAMAGE_CAL)==0
		and eg:IsExists(function(c,p) return c:IsSummonPlayer(1-p) end,1,nil,tp)
end
