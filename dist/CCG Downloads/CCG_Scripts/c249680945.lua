...
function s.thfilter(c) return c:IsSetCard(SET_GRAVINITY) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() end
...
function s.mvcon(e,tp)
	local c=e:GetHandler()
	return c:IsType(TYPE_TRAP) and c:GetSequence()<5 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
end
...
function s.mvop(e,tp)
	local c=e:GetHandler()
	if c:GetFlagEffect(TRANSFER_CALL)>0 then c:ResetFlagEffect(TRANSFER_CALL) s.thop(e,tp) end
	s.domove(c,tp)
end
function s.matval(e,sc) return sc:IsCode(LAPSIX,GALAXIX) end
