--A Stellaer Is Reborn
local s,id=GetID()
local STRING_ID=133549955
local SET_STELLAER=0xe40d
function s.initial_effect(c)
	--Special Summon 1 "Stellaer" monster from your GY
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE)
		and s.spfilter(chkc,e,tp) and aux.NecroValleyFilter()(chkc) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and aux.NecroValleyFilter()(tc) and s.spfilter(tc,e,tp) then
		Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)
	end
end
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
		and c:IsSetCard(SET_STELLAER) and c:IsType(TYPE_MONSTER)
		and c:IsReason(REASON_BATTLE+REASON_EFFECT) and not c:IsReason(REASON_REPLACE)
end
function s.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToRemove() and aux.NecroValleyFilter()(c) and eg:IsExists(s.repfilter,1,nil,tp)
...
function s.repop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if aux.NecroValleyFilter()(c) then
