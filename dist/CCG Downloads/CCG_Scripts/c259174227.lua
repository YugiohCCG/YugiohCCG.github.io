-- Farad, the Purple Ohmen
local s,id=GetID()
local STRING_ID=133174227
...
function s.additional_op(e,tp,zone)
	local spfilter=aux.NecroValleyFilter(s.spfilter)
	if Duel.IsExistingMatchingCard(spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,zone) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sg=Duel.SelectMatchingCard(tp,spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp,zone)
		if #sg>0 then
