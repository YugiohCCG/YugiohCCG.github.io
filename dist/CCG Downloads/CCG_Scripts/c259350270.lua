...
function s.thfilter(tc)
	return tc:IsSetCard(SET_OHMEN) and tc:IsFaceupEx() and tc:IsAbleToHand()
		and (not tc:IsLocation(LOCATION_GRAVE) or aux.NecroValleyFilter()(tc))
end
...
function s.efftg3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.NecroValleyFilter(Card.IsAbleToRemove),tp,0,LOCATION_GRAVE,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,1-tp,LOCATION_GRAVE)
end
function s.effop3(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(Card.IsAbleToRemove),tp,0,LOCATION_GRAVE,1,3,nil)
	if #g>0 then
