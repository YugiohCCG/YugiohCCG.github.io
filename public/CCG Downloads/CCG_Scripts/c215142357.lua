...
function s.prottg(e,c)
	return c:IsSetCard(SET_SHINING_BRIGADE) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c~=e:GetHandler()
end
function s.rmlimit(e,c,rp,r,re)
	return c:IsControler(e:GetHandlerPlayer()) and c:IsSetCard(SET_SHINING_BRIGADE) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c~=e:GetHandler()
end
...
function s.attg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.xyzfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(s.attfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil) end
end
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTRUCT)
	...
	-- detach
	if xyz:CheckRemoveOverlayCard(tp,1,REASON_EFFECT) and xyz:RemoveOverlayCard(tp,1,1,REASON_EFFECT) then
