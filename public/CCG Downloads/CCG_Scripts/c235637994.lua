...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	for tc in aux.Next(eg) do
		if tc:IsReason(REASON_EFFECT) and tc:IsPreviousLocation(LOCATION_HAND+LOCATION_ONFIELD)
			and (tc:IsPreviousSetCard(SET_CHRONO_SAUR) or tc:IsPreviousRace(RACE_DINOSAUR)) then
			Duel.RegisterFlagEffect(tc:GetPreviousControler(),id,RESET_PHASE+PHASE_END,0,1)
		end
	end
end
function s.spcon1(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()~=tp and Duel.IsBattlePhase() and Duel.GetFlagEffect(tp,id)>0
end
