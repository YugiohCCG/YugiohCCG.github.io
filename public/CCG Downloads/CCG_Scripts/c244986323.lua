...
function s.initial_effect(c)
	...
	--Xyz Summon
	Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x889),2,3,nil,nil,99)
	--Pendulum Effect
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(nil,tp,0,LOCATION_ONFIELD,1,nil)
		and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,LOCATION_GY,LOCATION_GY,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,nil,1,1-tp,LOCATION_ONFIELD)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,PLAYER_ALL,LOCATION_GY)
end
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)>0 then
		...
		local rg=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,LOCATION_GY,LOCATION_GY,1,1,nil)
		if #rg>0 then
			Duel.BreakEffect()
