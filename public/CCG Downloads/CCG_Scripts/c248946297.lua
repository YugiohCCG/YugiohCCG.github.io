...
function s.oppsummon(e,tp)
	...
	local tc=Duel.SelectMatchingCard(p,s.oppfilter,p,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,p):GetFirst()
	if not tc then return false,nil,false end
	local fromdeck=tc:IsLocation(LOCATION_DECK)
	if Duel.SpecialSummon(tc,0,p,p,false,false,POS_FACEUP_ATTACK)>0 then
		s.atkzero(e,tc)
