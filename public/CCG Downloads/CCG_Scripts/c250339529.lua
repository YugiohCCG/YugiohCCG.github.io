...
function s.initial_effect(c)
	--Fusion Material
	c:EnableReviveLimit()
	aux.AddFusionProcMixN(c,true,true,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_AQUAMARINE),2)
...
function s.fsop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #sg1>0 or (ce~=nil and #Duel.GetMatchingGroup(s.ffilter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,filter,chkf)>0) then
		...
		if sg1:IsContains(tc) and (ce==nil or not ce:SelectYesNo(tp)) then
