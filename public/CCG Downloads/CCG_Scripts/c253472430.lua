...
s.listed_series={0xc42}
function s.matfilter1(c)
	return c:IsFaceup() and c:IsSetCard(0xc42) and c:IsType(TYPE_SYNCHRO)
end
function s.matfilter2(c)
	return c:IsFaceup() and c:IsRace(RACE_ZOMBIE) and c:IsType(TYPE_TUNER)
end
function s.synfilter(c,mg)
	return c:IsSetCard(0xc42) and c:IsType(TYPE_SYNCHRO) and c:IsSynchroSummonable(nil,mg)
end
...
function s.ctcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,233499093),tp,LOCATION_MZONE,0,1,nil)
end
