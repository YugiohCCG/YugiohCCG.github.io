...
function s.revfilter(c)
	return c:IsSetCard(0xc43) and c:IsType(TYPE_SPELL) and not c:IsPublic()
end
...
function s.addfilter(c)
	return c:IsSetCard(0xc43) and not c:IsCode(id) and c:IsAbleToHand()
end
function s.setfilter(c)
	return c:IsSetCard(0xc43) and c:IsType(TYPE_QUICKPLAY) and c:IsSSetable()
end
...
function s.thfilter2(c)
	return c:IsSetCard(0xc43) and c:IsAbleToHand()
end
function s.thtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.thfilter2),tp,LOCATION_GRAVE,0,1,nil) end
