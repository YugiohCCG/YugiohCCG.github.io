...
function s.xyzfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_STELLAER) and c:IsType(TYPE_XYZ)
end
...
function s.xyzop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (c:IsRelateToEffect(e) and aux.NecroValleyFilter()(c) and c:IsCanOverlay() and tc and tc:IsRelateToEffect(e)
		and s.xyzfilter(tc) and not tc:IsImmuneToEffect(e)) then return end
	Duel.Overlay(tc,c)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD)
