...
function s.gyfilter(c)
	return s.isharpielady(c)
end
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	local mg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.gyfilter),tp,LOCATION_GRAVE,0,nil)
	...
	local g2=Duel.SelectTarget(tp,Card.IsFaceup,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,#g1,#g1,nil)
	g1:Merge(g2)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sc>0 then
		...
		if #fg>0 then
			...
			local rg=fg:Select(tp,math.min(sc,#fg),math.min(sc,#fg),nil)
